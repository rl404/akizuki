      ...
      <Card sx={{ borderLeft: `solid 5px ${UserStatusColor(data.userStatus)}` }}>
        <CardContent sx={{ p: 0, pl: 2, pr: 2, ':last-child': { paddingBottom: 0 } }}>
          <Grid container spacing={0.5} sx={{ alignItems: 'center' }}>
              ...
              <Link href={`${MAL_WEB_HOST}/manga/${data.id}`} target="_blank">
                ...
                >
                  ...
                  {data.status === 'currently_publishing' && (
                    <Typography sx={{ color: theme.palette.primary.main, display: 'inline' }}> — Publishing</Typography>
                  )}
                </Typography>
              </Link>
            </Grid>
            ...
            <Grid size={{ xs: 'grow', sm: 1 }} sx={{ textAlign: 'center' }} onClick={openChapterDialog}>
